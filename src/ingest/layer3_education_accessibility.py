...
def download_nces_school_directory(year: int) -> pd.DataFrame:
    ...
    if settings.NCES_CCD_PRELIM_URL:
        try:
            ...
            state_cols = [
                c for c in df.columns if c.upper() in ["ST", "STABBR", "STATE", "STATENAME"]
            ]
            if state_cols:
    ...
    logger.warning(
        "NCES directory download failed; returning empty dataframe (no synthetic fallback)."
    )
    return pd.DataFrame()
...
def _generate_fallback_school_directory(year: int) -> pd.DataFrame:
    ...
    with get_db() as db:
        ...
        county_data = pd.DataFrame(
            result.fetchall(), columns=["fips_code", "total_enrollment", "schools_total"]
        )
    ...
    for _, county_row in county_data.iterrows():
        ...
        for _, tract in county_tracts.iterrows():
            tract_share = (
                tract.get("population", 1) / total_pop if total_pop > 0 else 1 / len(county_tracts)
            )
                ...
                schools.append(
                    {
                        ...
                        "graduation_rate": (
                            np.random.uniform(0.75, 0.95) if school_type == "High" else None
                        ),
                        "data_year": year,
                    }
                )
...
def download_msde_proficiency_data(year: int) -> pd.DataFrame:
    ...
    logger.warning(
        "MSDE direct download not implemented - returning empty dataframe (no synthetic fallback)."
    )
    return pd.DataFrame()
...
def download_acs_school_age_population(year: int) -> pd.DataFrame:
    ...
    try:
        ...
        df["school_age_pop_5_17"] = (
            pd.to_numeric(df["B01001_004E"], errors="coerce").fillna(0)
            + pd.to_numeric(df["B01001_005E"], errors="coerce").fillna(0)  # 5-9 male
            + pd.to_numeric(df["B01001_006E"], errors="coerce").fillna(0)  # 10-14 male
            + pd.to_numeric(df["B01001_028E"], errors="coerce").fillna(0)  # 15-17 male
            + pd.to_numeric(df["B01001_029E"], errors="coerce").fillna(0)  # 5-9 female
            + pd.to_numeric(df["B01001_030E"], errors="coerce").fillna(  # 10-14 female
                0
            )  # 15-17 female
        ).astype(int)
...
def compute_tract_education_accessibility(
    tracts_df: pd.DataFrame, schools_df: pd.DataFrame, school_age_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    results = results.merge(
        school_age_df[
            ["tract_geoid", "school_age_pop_5_17", "school_age_pop_under_5", "total_population"]
        ],
        on="tract_geoid",
        how="left",
    )
    ...
    is_prek = (
        schools_df["has_prek"].fillna(False).values
        if "has_prek" in schools_df.columns
        else np.zeros(len(schools_df), dtype=bool)
    )
    ...
    school_quality = (
        schools_df["quality_score"].values
        if "quality_score" in schools_df.columns
        else np.ones(len(schools_df)) * 0.5
    )
    school_proficiency = (
        schools_df["avg_proficiency_pct"].values
        if "avg_proficiency_pct" in schools_df.columns
        else np.ones(len(schools_df)) * 50.0
    )
    ...
    tract_school_counts = (
        schools_df.groupby("tract_geoid")
        .agg({"nces_school_id": "count", "has_prek": lambda x: x.fillna(False).any()})
        .rename(
            columns={"nces_school_id": "total_schools_in_tract", "has_prek": "has_prek_program"}
        )
        .reset_index()
    )
    ...
    logger.info(
        f"Avg high-quality schools (30min): {results['high_quality_schools_30min'].mean():.1f}"
    )
...
def normalize_education_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if prof_accessible.max() > prof_accessible.min():
        df["school_quality_score"] = (prof_accessible - prof_accessible.min()) / (
            prof_accessible.max() - prof_accessible.min()
        )
    else:
    ...
    df["equity_adjusted_score"] = (
        df["education_accessibility_score"] * 0.9
    )  # Slight penalty placeholder
    ...
    logger.info(
        f"  Supply score: {df['school_supply_score'].mean():.3f} (std: {df['school_supply_score'].std():.3f})"
    )
...
def aggregate_to_county(tract_df: pd.DataFrame, schools_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for fips_code, group in tract_df.groupby("fips_code"):
        ...
        metrics = {
            ...
            "high_quality_schools_count": len(
                county_schools[
                    county_schools["quality_tier"].isin(["top_quartile", "above_median"])
                ]
            ),
            "top_quartile_schools_count": len(
                county_schools[county_schools["quality_tier"] == "top_quartile"]
            ),
            "schools_with_prek": (
                len(county_schools[county_schools["has_prek"] == True])
                if "has_prek" in county_schools.columns
                else 0
            ),
            ...
            "avg_ela_proficiency": (
                county_schools["ela_proficiency_pct"].mean()
                if "ela_proficiency_pct" in county_schools.columns
                else None
            ),
            "avg_math_proficiency": (
                county_schools["math_proficiency_pct"].mean()
                if "math_proficiency_pct" in county_schools.columns
                else None
            ),
            "avg_proficiency": (
                county_schools["avg_proficiency_pct"].mean()
                if "avg_proficiency_pct" in county_schools.columns
                else None
            ),
            "avg_graduation_rate": (
                county_schools[county_schools["school_type"] == "High"]["graduation_rate"].mean()
                if "graduation_rate" in county_schools.columns
                else None
            ),
            # Equity
            "frl_proficiency_gap": (
                county_schools["frl_proficiency_gap"].mean()
                if "frl_proficiency_gap" in county_schools.columns
                else None
            ),
            # Weighted scores
    ...
    logger.info(
        f"County avg education opportunity index: {county_df['education_opportunity_index'].mean():.3f}"
    )
...
def store_tract_education_accessibility(
...
def store_county_education_accessibility(
    df: pd.DataFrame, data_year: int, nces_year: int, acs_year: int
):
...
def calculate_education_accessibility_indicators(
    ...
    logger.info(
        f"Avg education opportunity index: {tract_df['education_opportunity_score'].mean():.3f}"
    )
...
def run_layer3_v2_ingestion(
    ...
    try:
        if multi_year:
            ...
            logger.info(
                f"Starting Layer 3 v2 MULTI-YEAR ingestion for years {years_to_fetch[0]}-{years_to_fetch[-1]}"
            )
        else:
        ...
        for year in years_to_fetch:
            ...
            try:
                tract_df, county_df, schools_df = calculate_education_accessibility_indicators(
                    data_year=year, nces_year=nces_year, acs_year=acs_year
                )
                ...
                if store_data and not tract_df.empty:
                    ...
                    log_refresh(
                        ...
                        metadata={
                            ...
                            "avg_education_opportunity": float(
                                tract_df["education_opportunity_score"].mean()
                            ),
                        },
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Layer 3 v2: Education Accessibility Analysis")
    parser.add_argument(
        "--year",
        type=int,
        default=None,
