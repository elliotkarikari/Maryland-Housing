...
def fetch_bps_permits(data_year: int) -> pd.DataFrame:
    ...
    df["permits_total"] = df[["units_101", "units_103", "units_104", "units_105"]].sum(
        axis=1, min_count=1
    )
...
def calculate_housing_indicators(df: pd.DataFrame, data_year: int) -> pd.DataFrame:
    ...
    permits_norm = (
        df["permits_per_1000_households"].rank(pct=True)
        if df["permits_per_1000_households"].notna().any()
        else None
    )
...
def run_layer4_ingestion(data_year: int = None, multi_year: bool = True):
    ...
    if multi_year:
        ...
        logger.info(
            f"Starting Layer 4 (Housing Elasticity) MULTI-YEAR ingestion for years {years_to_fetch[0]}-{years_to_fetch[-1]}"
        )
    else:
        years_to_fetch = [latest_year]
        logger.info(
            f"Starting Layer 4 (Housing Elasticity) single-year ingestion for {latest_year}"
        )
    ...
    if multi_year:
        ...
        logger.info(
            f"  Years requested: {years_to_fetch[0]}-{years_to_fetch[-1]} ({len(years_to_fetch)} years)"
        )
    ...
    log_refresh(
        ...
        status=(
            "success" if not failed_years or len(failed_years) < len(years_to_fetch) else "partial"
        ),
...
def main():
    ...
    parser.add_argument(
        "--single-year", action="store_true", help="Fetch only single year (default: multi-year)"
    )
