...
class CIPProjectItem(BaseModel):
    ...
    category: Optional[str] = Field(
        None, description="Category (e.g., 'schools', 'transportation')"
    )
    ...
    status: Optional[str] = Field(
        None, description="Project status (planned, approved, funded, in progress)"
    )
...
class DeliveryMetrics(BaseModel):
    ...
    projects_planned_prior_period: Optional[int] = Field(
        None, ge=0, description="Projects planned in prior CIP"
    )
    projects_completed_prior_period: Optional[int] = Field(
        None, ge=0, description="Projects actually completed"
    )
    ...
    def validate_completion_rate(cls, v, info):
        ...
        if (
            v is None
            and "projects_planned_prior_period" in info.data
            and "projects_completed_prior_period" in info.data
        ):
...
class CIPExtraction(BaseModel):
    ...
    total_capital_budget: Optional[float] = Field(
        None, ge=0, description="Total capital budget across all categories (USD)"
    )
    school_capital_budget: Optional[float] = Field(
        None, ge=0, description="Capital budget specifically for schools (USD)"
    )
    transport_capital_budget: Optional[float] = Field(
        None, ge=0, description="Capital budget for transportation infrastructure (USD)"
    )
    other_capital_budget: Optional[float] = Field(
        None, ge=0, description="Other capital spending not in above categories (USD)"
    )
    ...
    multi_year_commitments: Optional[int] = Field(
        None, ge=0, description="Count of projects spanning more than one fiscal year"
    )
    project_count: Optional[int] = Field(
        None, ge=0, description="Total number of capital projects listed"
    )
    ...
    delivery_metrics: Optional[DeliveryMetrics] = Field(
        None, description="Historical delivery performance if stated in document"
    )

    # Extraction metadata
    confidence: float = Field(
        ..., ge=0, le=1, description="Extraction confidence (0-1, from model or validator)"
    )
    extraction_notes: Optional[str] = Field(
        None, max_length=1000, description="Notes about extraction quality or ambiguities"
    )
    ...
    def validate_projects_count(cls, v, info):
        ...
        if v is not None and "project_count" in info.data:
            ...
            if stated_count is not None and len(v) > stated_count:
                raise ValueError(
                    f"More projects listed ({len(v)}) than stated count ({stated_count})"
                )
        return v
