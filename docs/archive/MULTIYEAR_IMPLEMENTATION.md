...
### Layer 1: Economic Opportunity (v2) ✅

**Data Source**: Census LODES (wage segments) + ACS 5-year
**Coverage**: 2017-2021 (5 years)
**Records**: 120 (24 counties × 5 years)

**Key Metrics**:
- High‑wage jobs accessible (30/45 min)
- Economic accessibility score (percentile)
- Local strength (sector diversity + stable sector share)
- Economic opportunity index (v1 + v2 blend)

**Multi-Year Enhancements**:
- Implemented year loop in `run_layer1_v2_ingestion()`
- Default multi‑year window ends at latest available data year
- Graceful per-year error handling
- Command-line flags: `--year`, `--single-year`

**File**: [src/ingest/layer1_economic_accessibility.py](../src/ingest/layer1_economic_accessibility.py:1034)
...
**File**: [src/ingest/layer2_accessibility.py](../src/ingest/layer2_accessibility.py:1316)
...
```bash
# Layer 1: Economic Opportunity v2 (multi-year)
python src/ingest/layer1_economic_accessibility.py  # Default: multi-year window
python src/ingest/layer1_economic_accessibility.py --year 2021 --single-year

# Layer 2: Mobility Accessibility v2 (snapshot)
python src/ingest/layer2_accessibility.py
